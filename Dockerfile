...
RUN Rscript -e 'remotes::install_version("outliers",upgrade="never", version = "0.14")'
...
RUN Rscript -e 'remotes::install_version("EBImage",upgrade="never", version = "4.30.0")'
...
EXPOSE 3838
CMD  ["R", "-e", "options('shiny.port'=3838,shiny.host='0.0.0.0');SVMshiny::run_app()"]
