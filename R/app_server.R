app_server <- function(input, output,session) {
  ...
  img1 <- callModule(mod_load_img_server, "load_img_ui_1", r, ix=reactive(1))
  ...
  pheno_norm = callModule(mod_norm_ch_server, "norm_ch_ui_2", img=img1, n=reactive(r$mod3$GFP), r)
  ...
  observeEvent(input$button, {
    ...
    if (count == 0 | count > length(r$img_dir$files)) {
      ...
      #callModule(mod_classify_loop_server, "mod_classify_loop_ui_1", r=r, classify=reactive("pos"), i=reactive(count))
    }
