...
mod_test_model_server <- function(input, output, session, r){
  ...
  # nseg = reactive({
  #   #browser()
  #   callModule(mod_n_segment_server, "n_segment_ui_a", nuc_norm=dapi_norm(), params=param(), r)})
  # 
  # seg_test = reactive({
  #   browser()
  #   paintObjects(nseg(), toRGB(dapi_norm()), opac=c(1,0.8),col=c("Green",NA),thick=TRUE,closed=FALSE)
  # })
  # output$model_img = renderPlot({
  #   plot(seg_test())})
  # 
  # dapi_norm = reactive({
  #   ch_n = param()$DAPI
  #   dapi_norm = callModule(mod_norm_ch_server, "norm_ch_ui_a", img=img(), n=reactive(ch_n), r)})
  # #browser()
  # pheno_norm = reactive({
  #   ch_n = param()$GFP
  #   callModule(mod_norm_ch_server, "norm_ch_ui_b", img=img(), n=reactive(ch_n), r)})
  # nseg = reactive({
  #   ll = param()
